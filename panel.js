...
/** 插件曾托管的 NapCat 服务名（仅作诊断识别，不再自动注册）。 */
const SVC_NAME = "dsh-napcat.service";
...
const REASON={napcat_not_running:'⚠️ NapCat 未启动——请先安装并启动 NapCat（见 README「前置依赖」），并在 NapCat WebUI 配置 127.0.0.1:3001 的 WebSocket 服务端',webui_not_found:'⚠️ 未找到 NapCat WebUI 配置或二维码文件（可在 ~/.dsh/qq-remote.json 设置 qrcodePath）',stale_qrcode:'⚠️ 二维码已过期，NapCat 正在重新生成…',waiting:'⏳ 等待二维码生成…（插件自动从 NapCat WebUI 获取，约 30-60 秒）'};
    ...
    ctx.effect(() => webServer.register({
      ...
      handler: async (_req, res) => {
        ...
        if (!svc || !svc.active) {
          json(res, {
            ok: false,
            message: `NapCat 未在运行（${svc ? svc.name + " 未启动" : "未找到 systemd 服务"}）。请先安装并启动 NapCat（见 README「前置依赖」），并在 NapCat WebUI 配置 127.0.0.1:3001 的 WebSocket 服务端；或在 ~/.dsh/qq-remote.json 配置 napcatServiceName / qrcodePath。`,
          });
          return;
        }
