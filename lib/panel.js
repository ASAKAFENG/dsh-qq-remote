...
import { createNapcat, SVC_NAME, NAPCAT_MIRRORS } from "./napcat.js";
...
export function createPanel(state) {
  ...
  const QR_CANDIDATES = () => {
    ...
    const list = [
      ...
      path.join(home, ".dsh", "napcat", "cache", "qrcode.png"),
  ...
  function writeWhitelist(list) {
    ...
    try {
      ...
      return { ok: false, error: `白名单写入失败: ${e.message}` };
    }
    config.allowedUsers = clean;
    return { ok: true, allowedUsers: clean };
  }

  /** 写入 NapCat 相关设置（merge 进 overlay + 热更新内存 config）。 */
  function writeNapcatSettings(patch) {
    const p = path.join(os.homedir(), ".dsh", "qq-remote.json");
    try {
      const d = fs.existsSync(p) ? JSON.parse(fs.readFileSync(p, "utf8")) : {};
      ...
      log.warn(`[qq-remote] NapCat 设置写入失败: ${e.message}`);
    }
...
refresh();setInterval(refresh,5000);
  ...
  function registerQqPanel() {
      ...
      handler: async (req, res) => {
        ...
        if (req.method === "POST") {
          ...
          try {
            ...
            const result = writeWhitelist(list);
            json(res, result);
          } catch (e) {
    ...
    // ── NapCat 下载源设置（官方 / 镜像 / 自定义） ──────────────
    ctx.effect(() => webServer.register({
      kind: "exact", path: "/qq-remote/napcat/settings",
      handler: async (req, res) => {
        if (req.method === "GET") {
          const cur = config.napcatDownloadUrl || NAPCAT_MIRRORS[0].url;
          const matched = NAPCAT_MIRRORS.find((m) => m.url === cur);
          json(res, {
            mirrors: NAPCAT_MIRRORS,
            current: cur,
            currentId: matched ? matched.id : "custom",
            napcatQQ: config.napcatQQ || "",
          });
          return;
        }
        if (req.method === "POST") {
          ...
          try {
            ...
            if (!url) {
              json(res, { ok: false, error: "下载地址不能为空" });
              return;
            }
            if (!/^https?:\/\//i.test(url) && !url.startsWith("file://")) {
              json(res, { ok: false, error: "地址需以 http(s):// 或 file:// 开头" });
              return;
            }
            const patch = { napcatDownloadUrl: url };
            if (parsed.napcatQQ !== undefined) patch.napcatQQ = String(parsed.napcatQQ).trim();
            writeNapcatSettings(patch);
            json(res, { ok: true, current: url, napcatQQ: patch.napcatQQ });
          } catch (e) {
            json(res, { ok: false, error: String(e.message ?? e) });
          }
          return;
